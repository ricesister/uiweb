...
public class ZBTGTest {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		//点击广告管理
		pageor.getWebElement("//*[@id=\"menu\"]/div/ul/li[6]/div").click();
		pageor.getWebElement("//*[@id=\"menu\"]/div/ul/li[6]/ul/li[2]").click();
		Thread.sleep(2000);
		//验证进入纸板团轮播图页面
		if(pageor.getWebElement("//*[@id=\"tHeader\"]").getText().contains("纸板团轮播图")) {
			System.out.println(DateUtil.getDate()+symbolSpilt+"进入纸板团轮播图界面");
		}
		...
		if(rows.size()==5) {
			System.out.println(DateUtil.getDate()+symbolSpilt+"纸板团轮播图位置已满，请先删除广告位！");
			driver.close();
			driver.quit();
			return;
		}
		...
		if(pageor.getWebElement("//*[@id=\"home\"]/div[2]/div/div/div/div[3]/div[1]/div[1]/div/div[1]/span").getText()
				.startsWith("编辑图片")){
			System.out.println(DateUtil.getDate()+symbolSpilt+"打开纸板团轮新增轮播图窗口");
		}
		...
		try {
			...
			System.out.println(DateUtil.getDate()+symbolSpilt+"上传纸板团轮轮播图附件成功！");
		}catch (Exception e) {
			e.printStackTrace();
			System.err.println(DateUtil.getDate()+symbolSpilt+"上传纸板团轮轮播图附件失败！");
			return;
		}
		...
		System.out.println(DateUtil.getDate()+symbolSpilt+"点击保存纸板团轮轮播图！");
		...
		if(lcols.get(3).getText().contains("baidu")) {
			System.out.println(DateUtil.getDate()+symbolSpilt+"成功保存纸板团轮轮播图！");
		}
