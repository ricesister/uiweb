...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    private static final String FILE_NAME = "cps-report.html";
    ...
    private void init() {
        ...
        htmlReporter.config().setDocumentTitle(LoggerUtil.getReportName());
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                while (logIterator.hasNext()){
                    ...
                    if(details.contains(LoggerUtil.getSpiltTimeAndMsg())){
                        String time = details.split(LoggerUtil.getSpiltTimeAndMsg())[0];
                        log.setTimestamp(getTime(Long.valueOf(time)));
                        log.setDetails(details.substring(time.length()+LoggerUtil.getSpiltTimeAndMsg().length()));
                    }else{
